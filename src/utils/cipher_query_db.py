...
def select_sqls(cond_dict, table_name, c, precision_str):
                ...
                if (l2>0): # 值：非空list
                    for value in item_values:
                        if isinstance(value, int):
                            ...
                            if precision_str=='is':
                                sub_str += item_key +' '+precision_str+' '+ '\'' + str(value) +'\''
                            elif precision_str=='like':
                                sub_str += item_key +' '+precision_str+' '+ '\'%' + str(value) +'%\''
                            l2=l2-1
                            if l2>0:
                                sub_str+=' or '
...
def query_db(db_file, table_name, cond_dict, precision_str):
    res_list = []
    try:
        conn = sqlite3.connect(db_file)
        conn.text_factory = str
        c = conn.cursor()
        c.execute("PRAGMA key='cud@b&12tk3'")
        
        
        sqls = select_sqls(cond_dict, table_name, c, precision_str)
...
if __name__ == '__main__':
    ...
    db_file = '../data/2017.db'
    
    res_list = query_db(db_file, area[0], cond_dict)
